...
export class SDKUtil {
    ...
    private generateIndexCode() {
        ...
        return [
            ...
            `        const result: ${responseDataTypeAnnotation} = await axios.post(url, requestBody, {`,
            '            ...axiosOptions,',
            '            headers: {',
            '                ...axiosOptions?.headers,',
            "                Authorization: (typeof credential === 'string' && credential.length > 0) ? credential : this?.options?.headers?.Authorization,",
            '            },',
            '        }).then((response) => ({ error: null, response: response?.data, headers: response?.headers, status: response?.status, statusText: response?.statusText })).catch((error) => ({ error, response: null, headers: error?.response?.headers, status: error?.response?.status, statusText: error?.response?.statusText }));',
