...
export class SDKUtil {
    ...
    private generateIndexCode() {
        ...
        return [
            ...
            '    ignoreCache?: boolean;',
            '}',
            `\nexport interface ${OPTIONS_NAME} extends RequestInit, ${REQUEST_OPTIONS_NAME} {`,
            ...
            '        const { getAuthorizationCredential, onResponse, ignoreCache, ...requestOptions } = this?.options;',
            '        const finalCustomOptions = _.merge({}, { ignoreCache }, { ignoreCache: options?.ignoreCache });',
            '        if (this.RESPONSE_CACHE_MAP.has(requestHash) && !finalCustomOptions?.ignoreCache) {',
            `            return this.RESPONSE_CACHE_MAP.get(requestHash) as ${responseDataTypeAnnotation};`,
            '        }',
            '        const credential = getAuthorizationCredential?.();',
            `        const result: ${responseDataTypeAnnotation} = await fetch(url, {`,
            '            ...requestOptions,',
            ...
            '            const headers = Object.entries(Array.from(response?.headers?.entries?.() ?? [])).reduce((result, [key, value]) => {',
            '                result[key] = value;',
            '                return result;',
            '            }, {});',
            '            if (!response?.ok) {',
