...
export class SDKUtil {
    ...
    private generateIndexCode() {
        ...
        return [
            ...
            '}[MethodTypeMapKeys];',
            `\nexport interface ${OPTIONS_NAME} extends RequestInit {`,
            ...
            `\n    public createRequest<T extends keyof ${METHOD_TYPE_MAP_NAME}>(url: T): (requestBody?: ${requestBodyTypeAnnotation}, options?: ${OPTIONS_NAME}) => Promise<${responseDataTypeAnnotation}> {`,
            "        if (typeof this.REQUEST_METHOD_MAP.get(url) !== 'function') {",
            `            this.REQUEST_METHOD_MAP.set(url, (requestBody?: ${requestBodyTypeAnnotation}, options?: ${OPTIONS_NAME}) => this.request.call(this, url, requestBody, options));`,
            '        }',
            `        return this.REQUEST_METHOD_MAP.get(url) as (requestBody?: ${requestBodyTypeAnnotation}, options?: ${OPTIONS_NAME}) => Promise<${responseDataTypeAnnotation}>;`,
            '    }',
            `\n    public async request<T extends keyof ${METHOD_TYPE_MAP_NAME}>(url: T, requestBody?: ${requestBodyTypeAnnotation}, options?: Omit<${OPTIONS_NAME}, 'getAuthorizationCredential'>): Promise<${responseDataTypeAnnotation}> {`,
            '        const requestHash = hash(requestBody ?? null);',
            "        const finalOptions = _.merge({}, this?.options, _.omit(options, ['getAuthorizationCredential']));",
            '        const { getAuthorizationCredential, onResponse, ignoreCache, timeout = 90000, prefix, ...requestOptions } = finalOptions;',
            '        if (this.RESPONSE_CACHE_MAP.has(requestHash) && !finalOptions?.ignoreCache) {',
            `            return this.RESPONSE_CACHE_MAP.get(requestHash) as ${responseDataTypeAnnotation};`,
            '        }',
            '        const credential = getAuthorizationCredential?.();',
            `        const result: ${responseDataTypeAnnotation} = await fetch((finalOptions?.prefix ?? '') + url, {`,
            '            ...requestOptions,',
